...
ModelTwoWindingsTransformer::evalZ(const double& t) {
  ...
  if (currStateIndex != getCurrentStepIndex()) {
    if (disableInternalTapChanger_ > 0.) {
      // external automaton
      ::TraceInfo() << DYNLog(TfoTapChange, id_, getCurrentStepIndex(), z_[currentStepIndexNum_]) << Trace::endline;
    } else {
      // internal automaton
      ::TraceInfo() << DYNLog(TfoTapChange, id_, z_[currentStepIndexNum_], getCurrentStepIndex()) << Trace::endline;
  ...
  if (doubleNotEquals(z_[currentLimitsDesactivateNum_], getCurrentLimitsDesactivate())) {
    ...
    ::TraceInfo() << DYNLog(DeactivateCurrentLimits, id_) << Trace::endline;
  }
  ...
  if (doubleNotEquals(z_[disableInternalTapChangerNum_], getDisableInternalTapChanger())) {
    ...
    ::TraceInfo() << DYNLog(DisableInternalTapChanger, id_) << Trace::endline;
  }
  ...
  if (doubleNotEquals(z_[tapChangerLockedNum_], getTapChangerLocked())) {
    ...
    if (z_[tapChangerLockedNum_] > 0)
      ::TraceInfo() << DYNLog(TapChangerLocked, id_) << Trace::endline;
  }
