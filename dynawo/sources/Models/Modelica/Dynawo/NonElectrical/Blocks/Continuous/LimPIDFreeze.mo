...
block LimPIDFreeze "P, PI, PD, and PID controller with limited output, anti-windup compensation, setpoint weighting, optional feed-forward and optional freezing of the state"
  ...
  parameter Blocks.Types.SimpleController controllerType = Blocks.Types.SimpleController.PID "Type of controller";
  parameter Real k(min = 0, unit = "1") = 1 "Gain of controller";
  parameter Types.Time Ti(min = Constants.small) = 0.5 "Time constant of Integrator block" annotation(
  ...
  parameter Real wd(min = 0) = 0 "Set-point weight for Derivative block (0..1)" annotation(
    Dialog(enable = controllerType == .Modelica.Blocks.Types.SimpleController.PD or controllerType == .Modelica.Blocks.Types.SimpleController.PID));
  parameter Real Ni(min = 100 * Constants.eps) = 0.9 "Ni*Ti is time constant of anti-windup compensation" annotation(
    Dialog(enable = controllerType == .Modelica.Blocks.Types.SimpleController.PI or controllerType == .Modelica.Blocks.Types.SimpleController.PID));
  parameter Real Nd(min = 100 * Constants.eps) = 10 "The higher Nd, the more ideal the derivative block" annotation(
    Dialog(enable = controllerType == .Modelica.Blocks.Types.SimpleController.PD or controllerType == .Modelica.Blocks.Types.SimpleController.PID));
  ...
  parameter InitPID initType = InitPID.DoNotUse_InitialIntegratorState "Type of initialization (1: no init, 2: steady state, 3: initial state, 4: initial output)" annotation(
    Evaluate = true,
  ...
  Blocks.Nonlinear.Limiter limiter(limitsAtInit = true, strict = true, uMax = yMax, uMin = yMin)  annotation(
    Placement(transformation(extent = {{70, -10}, {90, 10}})));
//  Blocks.Nonlinear.Limiter limiter(homotopyType = LimiterHomotopy.Linear, limitsAtInit = true, strict = true, uMax = yMax, uMin = yMin)  annotation(
//    Placement(transformation(extent = {{70, -10}, {90, 10}})));
  Blocks.Sources.Constant Dzero(k = 0) if not with_D annotation(
    Placement(transformation(extent = {{-40, 20}, {-30, 30}})));
  Blocks.Sources.Constant Izero(k = 0) if not with_I annotation(
  ...
  IntegratorSetFreeze I(initType = if initType == InitPID.SteadyState then Init.SteadyState else if initType == InitPID.InitialState or initType == InitPID.DoNotUse_InitialIntegratorState then Init.InitialState else Init.NoInit,k = unitTime / Ti, use_freeze = true, y_start = xi_start)  annotation(
    Placement(visible = true, transformation(origin = {-38, -50}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
protected
  parameter Boolean with_I = controllerType == Blocks.Types.SimpleController.PI or controllerType == Blocks.Types.SimpleController.PID annotation(
    Evaluate = true,
    HideResult = true);
  parameter Boolean with_D = controllerType == Blocks.Types.SimpleController.PD or controllerType == Blocks.Types.SimpleController.PID annotation(
    Evaluate = true,
    HideResult = true);
initial equation
...
equation
  ...
  connect(I.y, addPID.u3) annotation(
    Line(points = {{-26, -50}, {-20, -50}, {-20, -8}, {-12, -8}, {-12, -8}}, color = {0, 0, 127}));
